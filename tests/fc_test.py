...
@pytest.mark.parametrize("features", [30])
@pytest.mark.parametrize("input_shape", [(1, 100), (1,7), (2,2)])
#@pytest.mark.parametrize("input_shape", [(2,1)])
...
def test_fc(act_bits, weight_bits, features, input_shape,use_quantize, use_bias, use_relu):
    ...
    run_model_vs_tflite(fc_model, input_data, act_bits, use_quantize, tolerance= 0.0 if act_bits==16 else 0)
